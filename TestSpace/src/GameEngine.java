...
public abstract class GameEngine implements KeyListener, MouseListener, MouseMotionListener {
	...
	public static void createGame(GameEngine game, int framerate) {
		// Initialise Game
		game.init();
		// create a Clip object to avoid delays later
	...
	public void clearBackground(int width, int height) {
		// Clear background
		Color bg=mGraphics.getBackground();
		Color fg=mGraphics.getColor();
		mGraphics.setColor(bg);
		mGraphics.fillRect(0,0,width,height);
		mGraphics.setColor(fg);
	}
	...
	// This function draws a polygon (actually a shape so it can have bezier curves too)
	void drawPolygon(GeneralPath polygon) {
		// Draw a Polygon
		mGraphics.draw(polygon);
	}

	// This function draws a polygon using a line of width l
	void drawPolygon(GeneralPath polygon, double l) {
		// Set the stroke
		mGraphics.setStroke(new BasicStroke((float)l));
		// Draw a Polygon
		mGraphics.draw(polygon);
	...
	void drawSolidPolygon(GeneralPath polygon) {
		// Fill a Polygon
	...
	// This function converts an float to a String
